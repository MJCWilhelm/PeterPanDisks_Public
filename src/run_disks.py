'''
    To run
alpha 1e-3, Mdot = [1e-7, 1e-8, 1e-9, 1e-10], F = 10
alpha 1e-4, Mdot = [1e-7, 1e-8, 1e-9, 1e-10], F = 10
Same runs, but with greater disk masses (2e-3 -> 6e-3?)
'''
import numpy as np
...
accretion_rate = np.zeros((N_steps, number_of_disks))# | units.MSun/units.yr
...
print ("Running {a} disks for {b} Myr took {c} s, {d} min".format(a=number_of_disks, b=round(t_end.value_in(units.Myr), 5), c=end_main-start_main, d=(end_main-start_main)/60.))
...
np.savetxt(folder+'disp_times.dat', np.stack([stellar_masses.value_in(units.MSun), dispersion_time]).T, header='host star mass in MSun; time of disk dispersion in Myr')
